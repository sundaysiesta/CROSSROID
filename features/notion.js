	...
	async _doFetch() {
		...
		try {
			do {
				...
				console.log(
					`[NotionManager] Fetched page ${pageCount}: ${data.results.length} items (Total: ${totalFetched})`
				);
			...
			console.log(
				`[NotionManager] ✅ Fetch complete: ${
					map.size
				} valid users from ${pageCount} pages (Total items fetched: ${totalFetched}, Filtered: ${
					totalFetched - map.size
				} items without Discord ID)`
			);
			this.cache = map;
