...
const { TIME_REPORT_HOURS, TIME_REPORT_CHANNEL_ID } = require('../constants');
const { getDayType, isJapaneseHoliday, getHolidayName, getSchoolVacationType } = require('../utils');
...
時間と状況に応じて、40文字以内の短いメッセージを作成してください。`;
		...
		const aiMessage =
			completion.choices[0]?.message?.content || `${timeGreeting}だダラァ！${dateInfo}だけど今日も頑張るダラァ！`;
		return aiMessage;
	} catch (error) {
